...
public class UserController {
    ...
    public ResponseEntity<?> getCurrentUser(Authentication authentication) {
        ...
        return ResponseEntity.ok(authentication); // 현재 인증된 사용자 반환
    }
    ...
    public ResponseEntity<Map<String, Object>> getUserRole(@PathVariable String userId) {
        ...
        // 서비스에서 해당 사용자 역할 정보를 가져옴
        String role = userService.getUserRole(userId);
        response.put("role", role); // 역할 정보를 응답에 추가

        return ResponseEntity.ok(response); // 역할 반환
    }
}